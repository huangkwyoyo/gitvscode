...
FROM dws.dws_user_voice_sms_day_profile
...
FROM dws.dws_user_voice_sms_day_profile
WHERE biz_date = @biz_date
  AND (voice_usage_min < 0 OR sms_count < 0 OR mobile_data_usage_mb < 0 OR voice_rank_in_group < 0 OR group_user_count < 0);
...
FROM (
  SELECT (SELECT COUNT(*) FROM dwd.fact_usage_daily WHERE data_date = @biz_date) AS source_count,
         (SELECT COUNT(*) FROM dws.dws_user_voice_sms_day_profile WHERE biz_date = @biz_date) AS target_count
) counts;
