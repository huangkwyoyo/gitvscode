...
FROM dws.dws_channel_order_conversion_day
...
SELECT '主键唯一性校验' AS check_name,
       ...
       '业务主键不允许重复' AS remark
FROM (
  SELECT biz_date, channel_id, product_type, COUNT(*) AS duplicate_count
  FROM dws.dws_channel_order_conversion_day
  WHERE biz_date = @biz_date
  GROUP BY biz_date, channel_id, product_type
  HAVING COUNT(*) > 1
) duplicated_keys;
...
FROM dws.dws_channel_order_conversion_day
WHERE biz_date = @biz_date
  AND (biz_date IS NULL OR channel_id IS NULL OR product_type IS NULL);
...
FROM dws.dws_channel_order_conversion_day
WHERE biz_date = @biz_date
  AND (order_count < 0 OR paid_order_count < 0 OR cancelled_order_count < 0 OR payment_amount < 0 OR conversion_rate < 0 OR cancel_rate < 0 OR avg_pay_minutes < 0);
...
FROM (
  SELECT (SELECT COUNT(*) FROM dwd.fact_order_daily WHERE DATE(order_created_time) = @biz_date) AS source_count,
         (SELECT COUNT(*) FROM dws.dws_channel_order_conversion_day WHERE biz_date = @biz_date) AS target_count
) counts;
...
FROM dws.dws_channel_order_conversion_day
WHERE biz_date = @biz_date
  AND (channel_id IS NULL OR product_type IS NULL);
