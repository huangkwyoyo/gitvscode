...
FROM ads.ads_user_daily_value_tag
...
FROM ads.ads_user_daily_value_tag
WHERE biz_date = @biz_date
  AND (daily_revenue_fee < 0 OR month_billed_revenue_fee < 0 OR month_outstanding_amount < 0 OR last_30d_recharge_amount < 0 OR value_score < 0);
...
FROM (
  SELECT (SELECT COUNT(*) FROM dwd.fact_user_snapshot_daily WHERE data_date = @biz_date) AS source_count,
         (SELECT COUNT(*) FROM ads.ads_user_daily_value_tag WHERE biz_date = @biz_date) AS target_count
) counts;
