...
FROM dws.dws_user_month_lifecycle
...
SELECT '主键唯一性校验' AS check_name,
       ...
       '业务主键不允许重复' AS remark
FROM (
  SELECT data_month_date, user_id, COUNT(*) AS duplicate_count
  FROM dws.dws_user_month_lifecycle
  WHERE data_month_date = @month_start
  GROUP BY data_month_date, user_id
  HAVING COUNT(*) > 1
) duplicated_keys;
...
FROM dws.dws_user_month_lifecycle
WHERE data_month_date = @month_start
  AND (data_month_date IS NULL OR user_id IS NULL);
...
FROM dws.dws_user_month_lifecycle
WHERE data_month_date = @month_start
  AND (monthly_revenue_fee < 0 OR previous_month_revenue_fee < 0 OR mobile_data_usage_mb < 0 OR recharge_amount < 0 OR outstanding_amount < 0);
...
FROM (
  SELECT (SELECT COUNT(*) FROM dws.dws_user_month_summary WHERE data_month_date = @month_start) AS source_count,
         (SELECT COUNT(*) FROM dws.dws_user_month_lifecycle WHERE data_month_date = @month_start) AS target_count
) counts;
