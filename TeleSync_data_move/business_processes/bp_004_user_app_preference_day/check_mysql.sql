...
FROM dws.dws_user_app_preference_day
...
FROM dws.dws_user_app_preference_day
WHERE biz_date = @biz_date
  AND (top_app_usage_mb < 0 OR total_app_usage_mb < 0 OR top_app_usage_ratio < 0 OR total_page_view_count < 0);
...
FROM (
  SELECT (SELECT COUNT(DISTINCT user_id) FROM dwd.fact_dpi_usage_daily WHERE data_date = @biz_date) AS source_count,
         (SELECT COUNT(*) FROM dws.dws_user_app_preference_day WHERE biz_date = @biz_date) AS target_count
) counts;
