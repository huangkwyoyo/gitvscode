...
FROM ads.ads_enterprise_revenue_settlement_audit_day
...
SELECT '主键唯一性校验' AS check_name,
       ...
       '业务主键不允许重复' AS remark
FROM (
  SELECT biz_date, account_id, department_id, COUNT(*) AS duplicate_count
  FROM ads.ads_enterprise_revenue_settlement_audit_day
  WHERE biz_date = @biz_date
  GROUP BY biz_date, account_id, department_id
  HAVING COUNT(*) > 1
) duplicated_keys;
...
FROM ads.ads_enterprise_revenue_settlement_audit_day
WHERE biz_date = @biz_date
  AND (biz_date IS NULL OR account_id IS NULL OR department_id IS NULL);
...
FROM ads.ads_enterprise_revenue_settlement_audit_day
WHERE biz_date = @biz_date
  AND (revenue_fee < 0 OR settlement_fee < 0 OR post_settlement_fee < 0 OR tax_fee < 0 OR revenue_settlement_diff < 0);
...
FROM (
  SELECT (SELECT COUNT(*) FROM ods.ods_account_income_monthly WHERE DATE(data_time) = @biz_date) AS source_count,
         (SELECT COUNT(*) FROM ads.ads_enterprise_revenue_settlement_audit_day WHERE biz_date = @biz_date) AS target_count
) counts;
...
FROM ads.ads_enterprise_revenue_settlement_audit_day
WHERE biz_date = @biz_date
  AND (account_id IS NULL OR department_id IS NULL);
