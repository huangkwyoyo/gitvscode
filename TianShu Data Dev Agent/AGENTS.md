...
| 推荐表名 | 表选择是确定性决策 | `resolve_layer()` 函数 | `layer3_plan.py:_resolve_layer()` 查 ColumnBindingTable |
...
| 判断 G3 vs G2 | 层级选择是规则决策 | `resolve_layer()` 函数 | `layer3_plan.py:_resolve_layer()`——纯 if/else 逻辑 |
...
1. **Skill 输出仅作为分析素材**——最终分类判定必须由审查者根据契约文件定义执行
2. **问题必须逐条拆解**——Skill 发现的多个问题必须逐条独立归类，禁止整体套用分类
3. **C 类不可替代**——对 C 类问题，skill 输出不可替代完整风险评估报告
4. **硬边界优先**——Skill 分析过程中若发现触及 §7.2 中任一硬边界，立即升级为 C 类，终止 skill 辅助，输出完整风险评估报告
