...
```
总测试数：669 passed（归档时快照，当前以 `pytest tests/ -q` 为准）（0 failed）  ← v2.0 M2/M3/M4a/M4b + src/agent/ 直接单元测试 + SQL 安全口径收紧已纳入
执行时间：< 12s（全部测试）
...
| M3 SQL 样本执行 + 安全压实 | ✅ | `sandbox/executor.py`，529 passed零回归 |
