...
```
总测试数：475 passed（0 failed）  ← v2.0 M2/M3 + src/agent/ 直接单元测试 + SQL 安全口径收紧已纳入
执行时间：< 4s（全部测试）

v1.x 测试：143 passed（scripts/pipeline/ 确定性编译器）
v2.0 测试：325 passed（src/verify/、src/sandbox/、src/ir/、src/compile/、src/agent/）
```
...
| M3 SQL 样本执行 + 安全压实 | ✅ | `sandbox/executor.py`，475 测试零回归 |
