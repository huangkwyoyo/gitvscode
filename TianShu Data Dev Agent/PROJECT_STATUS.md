...
> **ARCHIVED：历史里程碑记录。** 当前实施状态以项目根目录 `AGENTS.md` §10 为权威事实源。
>
...
TianShu Data Dev Agent 是一个**AI 辅助数据开发工具**（v2.0 主链路）。v1.x 确定性数据生产管道保留为编译验证底座。

**v2.0 主链路**：需求输入 → Agent 生成 SQL + Spark DSL 双份草案 → 自动验证（7 项检查 + 交叉验证）→ Review Package 输出 → 人审决策。v2.0 主链路只生成和验证 SELECT/WITH SELECT 只读查询。

**v1.x 管道（legacy 底座）**：接收 YAML 需求说明书 → 经过 8 层管道处理 → 产出只读查询结果 + 验证报告。v1 Phase 2/DAG 保留写操作编译和静态校验能力（CTAS/INSERT/VIEW），但当前 Layer 6 执行器为 read_only=True，**未接入可写执行器，不可执行写入**。详见 §三能力矩阵。

**核心原则**：
- SQL 不是"写"出来的，是"编译"出来的
- v1.x: LLM 只在 Layer 1-2 参与（需求解析 + 指标消歧），Layer 3-8 是纯确定性代码
- v2.0: LLM 可生成代码草案，但所有草案必须经过 Validator → sample run → 人审
...
#### v1/v2 能力矩阵（关键——明确区分"可编译"与"可执行"）

| 链路 | 生成/编译 | 静态校验 | 当前执行能力 | 定位 |
|------|----------|---------|------------|------|
| **v1 Phase 1** (`run_pipeline.py`) | SELECT only（`compile_sql`） | `validate_sql`（6 项） | 只读执行（`read_only=True`） | legacy 入口 |
| **v1 Phase 2/DAG** | CTAS / INSERT / VIEW（`compile_operation`） | `validate_pipeline`（safety_tier=pipeline 写操作合规） | **未接可写执行器**（Layer 6 `read_only=True`） | 编译验证底座 |
| **v2 主链路** | SELECT + 只读 Spark（`dual_code_generator`） | v2 Validator（7 项，19 关键字） | 只读 sample run（sandbox `read_only=True`） | 当前主链路 |

> **关键区分**：
> - `safety_tier=pipeline` **不是运行时写权限开关**——它允许编译器和静态校验器处理写操作，但不赋予执行器写权限。
> - "可编译 CTAS" **不等于**"可以执行 CTAS"。
> - 当前项目**没有生产写入入口**。
> - Layer 6 执行器的 `read_only=True` 对所有 safety_tier 生效——即使 safety_tier=pipeline，执行器仍为只读。
> - 未来即使接入写入执行器，也必须单独设计审批、凭据、目标 schema、部署产物哈希和发布闸门。
...
| L6 | 执行层 | ✅ 完成（只读） | 禁止 | `layer6_execute.py`（`read_only=True`——对所有 safety_tier 生效） |
...
| 操作编译器（P6——5种操作 × 3方言） | ✅ | CTAS/INSERT OVERWRITE/INSERT INTO/CREATE VIEW + 方言差异。**可编译、可静态校验，但当前未接入可写执行器（Layer 6 `read_only=True`）** |
...
| 门禁 3：L5 DAG 校验 | ✅ | 环检测/依赖完整性/拓扑序/safety_tier合规（**编译时校验——pipeline tier 允许写操作编译通过，不等于运行时写权限**） |
...
```
总测试数：674 passed（归档时快照，当前以 `pytest tests/ -q` 为准）（0 failed）  ← v2.0 M2/M3/M4a/M4b/M5a/M5b-1 + B1幂等增强 + B3清理已纳入
执行时间：< 12s（全部测试）

v1.x 测试：143 passed（scripts/pipeline/ 确定性编译器）
v2.0 测试：486 passed（src/verify/、src/sandbox/、src/ir/、src/compile/、src/agent/、CLI）
```
...
| **M4b 状态机完整实现** | `test_src_agent_decision_manager.py` + 更新现有 + CLI | +52 | SUPERSEDED 自动转换 / artifact_hashes / 人审 CLI / golden fixtures |
| **M4c 跨 package 注册表** | `test_src_agent_package_registry.py` + CLI 新增 | +48 | package_registry / list/deps/status / SUPERSEDED 传播 / 一致性检查 |
...
- **M5a 查询内核与写入外壳分离**：复用现有 M5，补齐 SQL/Spark 双内核哈希、DeploymentManifest 契约、确定性部署外壳校验、`LOGIC_APPROVED`/`RELEASE_APPROVED` 语义分离、审批快照和篡改失效。当前仅生成与静态验证，不执行写入。
...
| M3 SQL 样本执行 + 安全压实 | ✅ | `sandbox/executor.py`，529 passed（历史快照） |
| M3 验证引擎串联 | ✅ | `verification_engine.py` 串起 checker + executor + cross_validation |
| 3 批次修复 + 安全压实 | ✅ | Batch 1-3 + G1/G2/G3 防御纵深缺口闭合 |
| `src/agent/` 模块直接单元测试 | ✅ | 7 文件（含 decision_manager.py）覆盖 7 个 M2/M3/M4b 核心模块 |
| **M4a 人审状态机最小实现** | ✅ | DecisionStatus enum + decision.yml + decision_log.yml + verification_summary.yml |
| **M4b 状态机完整实现** | ✅ | SUPERSEDED 自动转换 + artifact_hashes + decision_manager + 人审 CLI，581 测试（历史快照） |
| **M4c 跨 package 注册表** | ✅ | package_registry + list/deps/status + SUPERSEDED 传播 + 一致性检查，629 测试（历史快照） |
| **M5b-1 CTAS Sandbox + B1幂等增强 + B3清理** | ✅ | 674 测试（当前） |
...
- ✅ `src/agent/` 模块直接单元测试——142 个测试，475 总测试（历史快照）
- ✅ **Spark 只读样本执行**——`spark_executor.py` 12 层防御受控执行已实现（PySpark 不可用时 SKIPPED）
- ✅ **SQL/Spark 交叉验证**——`cross_validation.py` 7 维度比较已实现（Spark 不可用时 NOT_EXECUTED）
- ✅ **M5b-1 CTAS Sandbox**——`duckdb_ctas_executor.py` 一次性可写执行器 + 物化验证 + 幂等内容比较
- 完整 DAG 端到端测试——5 步管道 + 中间表管理
- Prompt 回归系统正式运行（`regression_baseline.yml` 基线，需 LLM API）

### v2.0 长期（P2——功能补全）

- 多表/JOIN CTAS Sandbox（M5b-2）
- INSERT/MERGE 写入策略
- PySpark 环境集成测试（当前 CI 环境无 PySpark，13 测试标记 skip）
- 人审状态机
- LLM 接入 M2 代码生成
- KEY_MERGE 增量策略
...
| 7 | **Spark executor 已实现**——`spark_executor.py` 12 层防御受控执行已实现，PySpark 不可用时 SKIPPED。交叉验证在 Spark 可用时产出 CONSISTENT_SAMPLE | 🟢 已修复 | 2026-06-19：Spark executor + 交叉验证已实现 |
