...
> 版本：v1.0 | 日期：2026-06-14 | 作者：Agent Team
>
> 本文档记录项目的整体状态——已完成、进行中、待开始、已知风险。
> 面向 Owner / 架构师 / 新加入团队成员，回答"项目现在到哪了"。
...
| 门禁 2：L5 SQL 校验 | ✅ | 6 项检查（黑名单/只读前缀/表引用/全限定名/JOIN白名单/日期合规） |
...
```
总测试数：135 passed（0 failed）
执行时间：< 0.1s（全部单元测试）
```

| 测试类 | 文件 | 数量 | 覆盖内容 |
|--------|------|------|---------|
| Phase 1 管道测试 | `tests/test_pipeline.py` | 19 | L1-L8 单条 SQL 全链路 + 端到端 |
| Phase 2 DAG 测试 | `tests/test_pipeline_phase2.py` | 25 | DAG 环检测/依赖完整性/拓扑序/安全层级 |
| P3 表达式编译器 | `tests/test_pipeline.py` | 31 | 10 类型 × 3 方言 + 嵌套 + SELECT 集成 |
| P4 窗口函数编译器 | `tests/test_pipeline.py` | 25 | 12 类型 + OVER 子句 + SELECT 集成 |
| P5 CTE 编译器 | `tests/test_pipeline.py` | 11 | 链式 CTE + C4 约束 + 参数传播 + SELECT 集成 |
| P6+P7 操作编译器+增量策略 | `tests/test_pipeline.py` | 24 | 策略解析 + 5操作×3方言 + 端到端 |
| **合计** | | **135** | |
...
| 文件 | 测试数 | 说明 |
|------|--------|------|
| `tests/test_pipeline.py` | 50 | Phase 1（19）+ P3 表达式编译器（31） |
...
| 2 | **ColumnBindingTable 依赖静态 backup**——指标注册依赖数据库 `meta.metric_definitions` + fallback 静态列表。P1-1 修复已添加加载状态追踪（`get_load_status()`）和诊断工具 | 🟡 中 | 已缓解——P1-1 修复已添加 `get_load_status()` 诊断函数，可实时查询事实源加载状态和漂移风险 |
| 3 | **KEY_MERGE 策略暂未实现**——需数据库 MERGE/UPSERT 支持 | 🟢 低 | 未来里程碑 |
| 4 | **DuckDB 分区覆盖降级为全量覆盖**——DuckDB 不支持 Hive 风格分区语法 | 🟢 低 | 设计如此——通过不同表名模拟分区 |
| 5 | **无完整 DAG 端到端测试**——PipelinePlan 单步测试覆盖，但多步 DAG 执行未测试 | 🟡 中 | 待操作编译器完成后 |
| 6 | `gold.fact_tif_payments` 缺少 dim_date JOIN 路径 | 🔴 已修复 | 2026-06-14：已添加 `fact_tif_payments ↔ dim_date` 白名单路径 |
