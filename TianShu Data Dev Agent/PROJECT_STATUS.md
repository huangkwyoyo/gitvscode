...
```
总测试数：326 passed（0 failed）  ← v2.0 M2/M3 测试已纳入
执行时间：< 2s（全部测试）

v1.x 测试：143 passed（scripts/pipeline/ 确定性编译器）
v2.0 测试：183 passed（src/verify/、src/sandbox/、src/ir/、src/compile/、src/agent/ 集成）
```
...
| TestDAGEndToEnd + 集成测试 | `test_pipeline_dag_e2e.py` | ~17 | DAG 编译 + 安全关键字 |
...
| M3 SQL 样本执行 + 安全压实 | ✅ | `sandbox/executor.py`，326 测试零回归 |
...
| 8 | **`src/agent/` 无直接单元测试**——M2/M3 模块仅通过集成测试间接覆盖 | 🟡 中 | v2.0 中期 |
