...
class TestLayer1Requirement:
    """第1层测试：需求解析"""

    def test_parse_valid_yaml(self):
        """解析有效的 YAML 需求"""
        req_path = PROJECT_ROOT / "fixtures" / "requirements" / "trip_daily_report.yml"
        req = parse_requirement(req_path)
        assert req.is_valid, f"解析失败: {req.validation_errors}"
        assert req.name == "2026年Q1每日行程量趋势"
        assert len(req.metrics) == 3

    def test_parse_missing_file(self):
        """解析不存在的文件"""
        req = parse_requirement(PROJECT_ROOT / "nonexistent.yml")
        assert not req.is_valid
        assert "文件不存在" in req.validation_errors[0]

    def test_parse_all_valid(self):
        """解析全部 3 个有效 fixture"""
        for name in ["trip_daily_report", "parking_daily_report", "crash_daily_report"]:
            path = PROJECT_ROOT / "fixtures" / "requirements" / f"{name}.yml"
            req = parse_requirement(path)
            assert req.is_valid, f"{name} 解析失败: {req.validation_errors}"
            assert len(req.metrics) > 0
            assert req.filters.date_range is not None
...
class TestLayer2Intent:
    ...
    def test_exact_metric_match(self):
        ...
        assert len(intent.metrics_requested) == 3
        assert intent.domain == "traffic"
        assert intent.confidence["metric_match"] == "high"
        assert len(intent.unresolved_metrics) == 0

    def test_fuzzy_metric_match(self):
        """模糊匹配（中文名 → 英文 registered_name）"""
        from scripts.pipeline.layer1_requirement import Requirement, MetricSpec, FilterSpec, OutputSpec
        req = Requirement(
            name="测试模糊匹配",
            metrics=[MetricSpec(name="行程量")],
            filters=FilterSpec(date_range=("2026-01-01", "2026-03-31")),
            output=OutputSpec(format="parquet"),
        )
        intent = build_intent(req)
        assert intent.is_valid
        assert intent.metrics_requested[0].registered_name == "trip_count"
        assert intent.metrics_requested[0].fuzzy_matched

    def test_invalid_metric_blocked(self):
        """无效指标名被阻挡"""
        from scripts.pipeline.layer1_requirement import Requirement, MetricSpec, FilterSpec, OutputSpec
        req = Requirement(
            name="测试无效指标",
            metrics=[MetricSpec(name="完全不存在的指标")],
            filters=FilterSpec(date_range=("2026-01-01", "2026-03-31")),
            output=OutputSpec(format="parquet"),
        )
        intent = build_intent(req)
        assert not intent.is_valid
        assert len(intent.unresolved_metrics) == 1
    ...
    def test_cross_domain_detection(self):
        ...
        req = Requirement(
            name="跨域测试",
            metrics=[
                MetricSpec(name="trip_count"),
                MetricSpec(name="crash_count"),
            ],
            filters=FilterSpec(date_range=("2026-01-01", "2026-03-31")),
            output=OutputSpec(format="parquet"),
        )
        intent = build_intent(req)
        assert intent.is_valid
        assert intent.domain == "cross_domain"


class TestLayer3SQLPlan:
    """第3层测试：SQLPlan 构造"""

    def test_g3_single_table_plan(self):
        """G3 单表 SQLPlan"""
        req = parse_requirement(
            PROJECT_ROOT / "fixtures" / "requirements" / "trip_daily_report.yml"
        )
        ...
        assert plan.source_layer == "g3"
        assert plan.join_graph is not None
        assert plan.join_graph.primary.table == "gold.dws_daily_trip_summary"
        assert len(plan.join_graph.joins) == 0
        assert len(plan.column_bindings) == 3

    def test_cross_domain_join_graph(self):
        """跨域 JOIN 图构造"""
        from scripts.pipeline.layer1_requirement import Requirement, MetricSpec, FilterSpec, OutputSpec
        req = Requirement(
            name="跨域 JOIN 测试",
        ...
        assert len(plan.join_graph.joins) == 1  # trip ↔ crash via date
        # 不校验具体谁是主表（取决于 set 迭代顺序），只校验两张表都在 JOIN 图中
        all_tables = {plan.join_graph.primary.table}
        for j in plan.join_graph.joins:
            all_tables.add(j.table)
        assert "gold.dws_daily_trip_summary" in all_tables
        assert "gold.dws_daily_crash_summary" in all_tables
...
class TestEndToEndPipeline:
    ...
    这些测试走完 Layer 1-5，确认 SQL 可以被编译和校验。
    实际 SQL 执行需要 TianShu DuckDB 存在且可访问。
    """

    def test_trip_daily_pipeline_dry_run(self):
        """行程日报：全链路线到 Layer 5"""
        req = parse_requirement(
            PROJECT_ROOT / "fixtures" / "requirements" / "trip_daily_report.yml"
        )
        ...
        validation = validate_sql(sql, plan)
        assert validation.passed

    def test_parking_daily_pipeline_dry_run(self):
        """罚单日报：全链路到 Layer 5"""
        req = parse_requirement(
            PROJECT_ROOT / "fixtures" / "requirements" / "parking_daily_report.yml"
        )
        intent = build_intent(req)
        plan = construct_sqlplan(intent)
        sql, _ = compile_sql(plan)
    ...
    def test_crash_daily_pipeline_dry_run(self):
        """事故日报：全链路到 Layer 5"""
        req = parse_requirement(
            PROJECT_ROOT / "fixtures" / "requirements" / "crash_daily_report.yml"
        )
        intent = build_intent(req)
        plan = construct_sqlplan(intent)
        sql, _ = compile_sql(plan)
