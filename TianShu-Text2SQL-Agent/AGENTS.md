...
#### C 类（ARCHITECTURE-RISK）—— 系统核心边界风险
...
> **辅助工具规则**：对于 B 类（复杂设计审查）和 C 类（架构边界审查），优先使用 `superpowers` / `review` skill 辅助拆解问题；但最终分类和结论仍必须落到 CRCS 的 A/B/C 体系，skill 输出不替代人工判定。
...
**使用约束**：
1. Skill 输出仅作为分析素材，最终分类判定必须由审查者根据 9.2 节定义执行
2. Skill 发现的问题必须逐条拆解并独立分类，禁止整体套用 skill 的分类结论
3. 对 C 类问题，skill 输出不可替代 9.2 节要求的风险评估报告（边界检查清单 + 至少 2 种安全替代方案）
