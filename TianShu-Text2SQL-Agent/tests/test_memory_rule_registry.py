...
class TestProposedNotBlocking:
    ...
    def test_all_rules_are_proposed_not_active(self):
        """除已晋升的 TA-R018/TA-R019/TA-R020 外，其余规则必须为 proposed"""
        rules = load_rules(RULES_YAML_PATH)
        active_rules = {"TA-R018", "TA-R019", "TA-R020"}  # Step 8b/Step 26/Step 26b 已晋升为 active
        for rule in rules:
            rid = rule["rule_id"]
    ...
    def test_no_rule_is_blocking(self):
        """除已晋升的 TA-R018/TA-R019/TA-R020 外，其余规则 blocking 必须为 false"""
        rules = load_rules(RULES_YAML_PATH)
        blocking_rules = {"TA-R018", "TA-R019", "TA-R020"}  # Step 8b/Step 26/Step 26b 已晋升为 blocking=true
        for rule in rules:
            rid = rule["rule_id"]
