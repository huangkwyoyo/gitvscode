...
> **关键**：此路由表旨在防止 Agent 将错误事实写入记忆系统。在写入任何 `docs/memory/` 条目之前，必须完成以下交叉验证。

| 顺序 | 文件 | 为什么必须读 |
|---|---|---|
| 1 | `docs/warehouse/database_design/`（相关层的设计文档） | 交叉验证记忆中的事实是否与最高事实源一致——**记忆条目中引用的表名、字段名必须在 database_design 中真实存在** |
| 2 | `docs/memory/规则来源索引.md` | 检查是否已有类似条目，避免重复或矛盾 |
| 3 | `docs/memory/变更复盘模板.md` | 确认条目格式、置信等级、结构化来源、必填字段完整 |
| 4 | `docs/standards/可信等级定义.md` | **必须先读**——确认理解各级置信等级的含义和初始写入时不得高于 L3 的硬约束 |
| 5 | `docs/warehouse/gold/AGENTS.md`（如涉及 Gold） | 检查记忆中的规则是否与现有分层规则冲突 |

**写入时的硬约束**：
- **初始置信等级不得高于 L3（Assumption）**——除非有 database_design 或 DESCRIBE 结果等硬证据支撑
- **新记忆默认状态为 `proposed`**（非 `active`）——需经 Review Agent 或人工确认后才能转为 `active`
- **必须至少填写 1 个结构化来源字段**（来源文档/PR/Commit/数据/人工确认）

写入后必须运行：
```bash
python scripts/quality/check_memory_update.py --check-content
```
确认必填字段完整、结构化来源完整、内容长度达标、无重复、数据库交叉验证通过。
...
> 本节定义 Agent 读取记忆时，如何根据置信等级（详见 `docs/standards/可信等级定义.md`）调整引用行为。

Agent 读取 `docs/memory/经验复盘.md` 中的条目时，必须根据条目头部的 `置信等级` 字段执行以下规则：

| 置信等级 | 含义 | Agent 处理规则 |
|---------|------|---------------|
| **L5**（Verified Fact） | 经多源交叉验证 + 人工确认 | 可直接引用，可作为写入 AGENTS.md 规则的依据 |
| **L4**（Fact） | 基于 DESCRIBE / 数据画像 / 代码审查确认 | 可直接引用，可作为写入检查脚本的依据；写入 AGENTS.md 规则前需额外确认 |
| **L3**（Assumption） | 合理推导但未经独立验证 | 可引用但必须标注"据推测"；**不得**作为写入 AGENTS.md 规则或检查脚本的唯一依据 |
| **L2**（Hypothesis） | 有依据的猜测 | 仅作参考，**不得**作为决策依据，**不得**写入任何正式规则或脚本 |
| **L1**（Human Reviewed） | 经人工确认但证据链不完整 | 可引用但优先级低于 L4/L5；若与 L4/L5 条目冲突，以 L4/L5 为准 |

**状态约束**：
- `proposed` 状态的条目：可读取但不得作为决策唯一依据——必须等待其转为 `active`
- `disputed` 状态的条目：**禁止引用**，跳过该条目
- `superseded` 状态的条目：阅读替代条目，跳过本条（除非需要追溯历史版本）
- `archived` 状态的条目：可跳过

**冲突处理**：当两条 active 条目内容矛盾时，优先采信置信等级更高的条目。若等级相同，以日期更新的为准。并在产出中标注冲突和选择依据。
